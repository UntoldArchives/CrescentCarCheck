    ...
    <section id="how-it-works" className="bg-surface section-padding relative scroll-mt-24">
      ...
      <div ref={ref} className="reveal container-wide relative">
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-3 gap-8 mt-12">
          <div
            className="hidden md:block absolute top-12 left-[16.66%] right-[16.66%] border-t-2 border-dashed border-border z-0"
            aria-hidden="true"
          />
          {STEPS.map((step) => (
            <div key={step.num} className="relative z-10">
              <span className="block text-7xl font-black text-border select-none leading-none" aria-hidden="true">
                {step.num}
              </span>
              ...
              <h3 className="text-xl font-bold text-text-primary mt-4">{step.title}</h3>
